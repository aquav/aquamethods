...
*/
	...
	public String getOutfit(@PathVariable("personId") int personId,
		...
		for (Event event : currentOutfitEventList) {
			EventForm eventForm = new EventForm();
			eventForm.setId(event.getId());
			eventForm.setName(event.getName());
		...
		outfitForm.setOutfitEvents(eventFormList);
	...
	@RequestMapping(value="/{personId}/outfit/{outfitId}/event/{eventId}", method=RequestMethod.POST)
